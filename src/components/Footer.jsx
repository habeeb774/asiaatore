        ...
        {footerBanners.length > 0 && (
          <div className="mb-6 flex flex-wrap items-center justify-center gap-4">
            {footerBanners.slice(0,3).map(b => (
              <a key={b.id} href={b.linkUrl || '#'} className="inline-flex items-center gap-2 text-sm text-gray-700 hover:text-primary-red transition" onClick={()=>{ try { api.marketingTrackClick('banner', b.id); } catch{} }}>
                {b.image && <img src={b.image} alt={b.title?.ar || b.title?.en || ''} className="h-10 w-auto rounded object-cover" />}
                <span>{b.title?.ar || b.title?.en}</span>
              </a>
            ))}
          </div>
        )}
          ...
          <div className="mb-6 flex flex-wrap items-center justify-center gap-4">
            {appLinks.map(l => (
              <a key={l.id} href={l.url} className="inline-flex items-center gap-2 text-xs px-3 py-2 bg-white border rounded-md shadow-sm hover:shadow transition" rel="noopener noreferrer" target="_blank" onClick={()=>{ try { api.marketingTrackClick('appLink', l.id); } catch{} }}>
                ...
                <span>{l.label?.ar || l.label?.en || l.platform}</span>
              </a>
            ))}
          </div>
        )}
