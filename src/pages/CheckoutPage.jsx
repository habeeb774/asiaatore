...
const CheckoutPage = () => {
  ...
  const loadAddress = () => {
    ...
    return { name: user?.name || '', email: user?.email || '', country: 'SA', city: '', line1: '', phone: '' };
  };
  ...
  if (!cartItems?.length && !open.complete) {
    return <div className="container-custom px-4 py-12"><h2 className="text-xl font-bold">السلة فارغة</h2></div>;
  }
  ...
  <div className="mb-4 border rounded overflow-hidden">
        ...
        <div className="grid gap-4 max-w-xl p-4">
          <input className="border px-3 py-2" placeholder="الاسم" value={addr.name} onChange={e=>setAddr(a=>({...a,name:e.target.value}))} />
          {errors.name && <div className="text-xs text-red-600">{errors.name}</div>}
          <input className="border px-3 py-2" placeholder="البريد" value={addr.email} onChange={e=>setAddr(a=>({...a,email:e.target.value}))} />
          {errors.email && <div className="text-xs text-red-600">{errors.email}</div>}
          <div className="grid grid-cols-2 gap-4">
            <input className="border px-3 py-2" placeholder="الدولة" value={addr.country} onChange={e=>setAddr(a=>({...a,country:e.target.value}))} />
            <input className="border px-3 py-2" placeholder="المدينة" value={addr.city} onChange={e=>setAddr(a=>({...a,city:e.target.value}))} />
          </div>
          {errors.city && <div className="text-xs text-red-600">{errors.city}</div>}
          <input className="border px-3 py-2" placeholder="العنوان" value={addr.line1} onChange={e=>setAddr(a=>({...a,line1:e.target.value}))} />
          {errors.line1 && <div className="text-xs text-red-600">{errors.line1}</div>}
          <input className="border px-3 py-2" placeholder="الهاتف" value={addr.phone} onChange={e=>setAddr(a=>({...a,phone:e.target.value}))} />
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input className="border px-3 py-2 flex-1 text-sm" placeholder="رمز القسيمة" value={coupon} onChange={e=>setCoupon(e.target.value)} />
