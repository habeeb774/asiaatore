...
export const LanguageProvider = ({ children, initialLocale }) => {
  ...
  const defaultLocale = (pathLocale && ['ar','en'].includes(pathLocale)) ? pathLocale : (initialLocale || i18n?.language || 'ar');
  ...
  const setLocale = useCallback((nextLocale) => {
    ...
    if (segments[0] && ['ar','en'].includes(segments[0])) {
      segments.shift();
    }
    if (nextLocale !== 'ar') {
  ...
  const value = useMemo(() => ({ locale, setLocale, t, available: ['ar','en'] }), [locale, t, setLocale]);
