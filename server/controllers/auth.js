...
router.post('/login', async (req, res) => {
  try {
    ...
    if (DEV_AUTH_ENABLED) {
      const dev = DEV_USERS.find(u => u.email === identifier && u.pass === password);
      if (dev) {
    ...
    if (!user) {
      if (process.env.DEBUG_LOGIN === '1') console.warn('[LOGIN] User not found for identifier:', identifier);
    ...
    if (!match) {
      if (process.env.DEBUG_LOGIN === '1') console.warn('[LOGIN] Bad password for identifier:', identifier);
      const detailed = process.env.DEBUG_ERRORS === 'true' || process.env.NODE_ENV !== 'production';
    ...
    if (user.mfaEnabled) {
      if (!mfaCode) {
        return res.status(401).json({ ok: false, error: 'MFA_REQUIRED', message: 'MFA code is required' });
      }
      const verified = speakeasy.totp.verify({ secret: user.mfaSecret, encoding: 'base32', token: mfaCode });
      if (!verified) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (email) {
      ...
      if (existingEmail) return res.status(409).json({ ok:false, error: 'EMAIL_EXISTS' });
    }
    if (phone) {
      const existingPhone = await prisma.user.findFirst({ where: { phone } });
      if (existingPhone) return res.status(409).json({ ok:false, error: 'PHONE_EXISTS' });
    }
    ...
    try {
      ...
      const data = { password: hashed };
      if (email && (!allowed.size || allowed.has('email'))) data.email = email;
    ...
    // Send email verification link (optional in dev) - only if email is provided
    if (email) {
  ...
  const tokenHash = sha256(rawRefresh);
    ...
    return res.status(201).json({ ok:true, accessToken, user: { id: created.id, role: created.role, email: created.email, name: created.name, phone: created.phone } });
  } catch (e) {
...
router.post('/mfa/setup', attachUser, async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ ok: false, error: 'UNAUTHENTICATED' });
    const user = await prisma.user.findUnique({ where: { id: req.user.id } });
    if (!user) return res.status(404).json({ ok: false, error: 'USER_NOT_FOUND' });
    if (user.mfaEnabled) return res.status(400).json({ ok: false, error: 'MFA_ALREADY_ENABLED' });
    const secret = speakeasy.generateSecret({ name: `MyStore (${user.email})`, issuer: 'MyStore' });
    // Save secret temporarily (not enabled yet)
    await prisma.user.update({ where: { id: user.id }, data: { mfaSecret: secret.base32 } });
    const qrCodeUrl = await qrcode.toDataURL(secret.otpauth_url);
    res.json({ ok: true, secret: secret.base32, qrCodeUrl });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'MFA_SETUP_FAILED', message: e.message });
  }
});

// MFA Enable: Verify code and enable
router.post('/mfa/enable', attachUser, async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ ok: false, error: 'UNAUTHENTICATED' });
    const { code } = req.body || {};
    if (!code) return res.status(400).json({ ok: false, error: 'MISSING_CODE' });
    const user = await prisma.user.findUnique({ where: { id: req.user.id } });
    if (!user || !user.mfaSecret) return res.status(400).json({ ok: false, error: 'MFA_NOT_SETUP' });
    const verified = speakeasy.totp.verify({ secret: user.mfaSecret, encoding: 'base32', token: code });
    if (!verified) return res.status(400).json({ ok: false, error: 'INVALID_CODE' });
    await prisma.user.update({ where: { id: user.id }, data: { mfaEnabled: true } });
    audit({ action: 'user.mfa_enabled', entity: 'User', entityId: user.id, userId: user.id });
    res.json({ ok: true, enabled: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'MFA_ENABLE_FAILED', message: e.message });
  }
});

// MFA Disable
router.post('/mfa/disable', attachUser, async (req, res) => {
  try {
    if (!req.user) return res.status(401).json({ ok: false, error: 'UNAUTHENTICATED' });
    const user = await prisma.user.findUnique({ where: { id: req.user.id } });
    if (!user) return res.status(404).json({ ok: false, error: 'USER_NOT_FOUND' });
    await prisma.user.update({ where: { id: user.id }, data: { mfaEnabled: false, mfaSecret: null } });
    audit({ action: 'user.mfa_disabled', entity: 'User', entityId: user.id, userId: user.id });
    res.json({ ok: true, disabled: true });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'MFA_DISABLE_FAILED', message: e.message });
  }
});
