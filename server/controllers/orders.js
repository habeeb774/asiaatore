...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      await sendOrderStatusNotification(created, 'pending');
    } catch (e) {
      console.error('[NOTIFICATION] Failed to send order confirmation:', e.message);
    }
  ...
  try {
    ...
    if (parsed.data.status && parsed.data.status !== existing.status) {
      try {
        ...
        console.error('[NOTIFICATION] Failed to send status update:', e.message);
      }
    }
