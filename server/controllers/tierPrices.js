...
router.get('/products/:productId/tier-prices', async (req,res) => {
  ...
  } catch(e){ req.log?.error({ err: e }, 'Failed to list tier prices'); res.status(500).json({ error:'FAILED_LIST_TIERS', message:e.message }); }
});
...
router.post('/products/:productId/tier-prices', requireAdmin, async (req,res) => {
  ...
  } catch(e){ if (e.code==='P2002') return res.status(409).json({ error:'DUPLICATE_TIER' }); req.log?.error({ err: e }, 'Failed to create tier price'); res.status(400).json({ error:'FAILED_CREATE_TIER', message:e.message }); }
});
