...
// Initialize Prisma soft-delete middleware now that Prisma client and DB status are known.
try {
  const { initSoftDelete } = await import('./db/softDeleteMiddleware.js');
  try { await initSoftDelete(prisma); console.log('[SoftDelete] Middleware initialized (if supported by schema).'); } catch (e) { console.warn('[SoftDelete] init failed:', e.message); }
} catch (e) {
  // Non-fatal: optional middleware
  console.warn('[SoftDelete] Could not load middleware:', e.message);
}
