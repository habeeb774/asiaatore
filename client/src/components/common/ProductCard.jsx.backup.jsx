...
const ProductCard = ({ product }) => {
  ...
  // Prefer WebP fallback when available (server writes .webp for uploads). Simple conversion: replace common raster extensions with .webp
  const toWebp = (u) => {
    if (!u || typeof u !== 'string') return u;
    if (u.includes('.webp')) return u;
    return u.replace(/\.(jpe?g|png)(\?.*)?$/i, '.webp$2');
  }
  const imageWebp = toWebp(mainImage);
    ...
    <div className="product-card" aria-label={nameText}>
      <Link to={`/products/${product.id}`} className="product-image" aria-label={`عرض ${nameText}`}>
        <LazyImage
          src={mainImage}
          alt={nameText}
          webpSrcSet={imageWebp}
          // Let browser choose size; this matches the grid breakpoints used in CSS
          sizes="(min-width: 1024px) 20vw, (min-width: 768px) 33vw, 50vw"
          // Reserve an aspect ratio to avoid layout shifts (4:3)
