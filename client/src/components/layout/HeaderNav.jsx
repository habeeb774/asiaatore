...
export const HeaderNav = React.memo(function HeaderNav({ className = '' }) {
  ...
  const logoSrc = useMemo(() => {
    try {
      // Prefer explicit settings; fallback to project public asset
      return setting?.logoUrl || setting?.logo || '/images/site-logo.jpg';
    } catch { return '/images/site-logo.jpg'; }
  }, [setting]);
  ...
  return (
    <header dir="rtl" className={`w-full sticky top-0 z-[1100] ${headerHeight} bg-white/90 dark:bg-slate-900/85 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b dark:border-slate-800 transition-shadow ${scrolled ? 'shadow-sm' : ''} ${className}`}>
      ...
      <div className="max-w-[1100px] mx-auto w-full h-full px-4 md:px-6 flex items-center justify-between gap-3">
        ...
        <Link to="/" className="flex items-center gap-3">
          ...
          <img
            src={logoSrc}
            alt={(setting && (setting.siteNameAr || setting.siteNameEn)) || 'Logo'}
            ...
            onError={(e) => {
              try {
                ...
                // One-time swap to SVG fallback if JPG/PNG missing
                if (!el.dataset.fallbackTried) {
                  el.dataset.fallbackTried = '1';
                  el.src = '/images/site-logo.svg';
                } else {
