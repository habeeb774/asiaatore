...
export default function AccountSecurity(){
  ...
  async function load(){
    try {
      setLoading(true);
      const res = await api.request ? api.request('/auth/me') : fetch('/auth/me').then(r=>r.json());
      const data = res.ok ? res : await fetch('/auth/me').then(r=>r.json());
  ...
  async function savePhone(){
    try {
      setErr(null); setInfo('');
      const res = await fetch('/auth/me', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ phone }) });
