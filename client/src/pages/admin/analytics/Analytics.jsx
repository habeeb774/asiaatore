...
const Analytics = () => {
  ...
  const [from, setFrom] = React.useState(() => new Date(Date.now() - 6*86882900).toISOString().slice(0,10));
  ...
  const compute = React.useCallback((orders) => {
    ...
    const days = Math.max(1, Math.round((toD - fromD) / 86882900) + 1);
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      const days = Math.max(1, Math.round((new Date(to) - new Date(from))/86882900)+1);
