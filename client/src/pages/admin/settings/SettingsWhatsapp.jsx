      ...
      <fieldset style={{display:'grid', gap:8, border:'1px solid #e2e8f0', borderRadius:8, padding:12}}>
        ...
        <div style={{display:'grid', gap:8}}>
          ...
          <div style={{display:'flex', gap:8, alignItems:'center'}}>
            ...
            {waDiag && (
              <div className="text-xs" style={{display:'grid', gap:4}}>
                ...
                <div>جاهزية التهيئة: <strong style={{color: ((waDiag.configured && waDiag.enabled) ? 'var(--color-green-900)' : 'var(--color-red-900)')}}>{(waDiag.configured && waDiag.enabled) ? 'جاهز' : 'غير مكتمل'}</strong></div>
                  ...
                  <div style={{display:'grid', gridTemplateColumns:'repeat(2, minmax(120px,1fr))', gap:6}}>
                    {Object.entries(waDiag.meta || waDiag.twilio).map(([k,v]) => (
                      <div key={k} className="text-[11px]" style={{opacity:.85}}>
                        <span style={{fontWeight:700}}>{k}</span>: <span style={{color: v ? 'var(--color-green-900)' : 'var(--color-red-900)'}}>{String(v)}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div style={{display:'grid', gap:6, borderTop:'1px dashed #e2e8f0', paddingTop:8}}>
            ...
            {waSendResult && (
              <div className="text-[12px]" style={{color: waSendResult.ok !== false ? 'var(--color-green-900)' : 'var(--color-red-900)'}}>
