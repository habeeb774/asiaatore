...
import { CheckoutSkeleton } from '../../components/shared/PageSkeletons';

const STRIPE_AVAILABLE = Boolean(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY);

const CheckoutPage = () => {
  const { cartItems, clearCart, updateQuantity, removeFromCart, maxPerItem } = useCart() || {};
  const { user } = useAuth() || {};
  ...
  const enabledPayments = useMemo(() => {
    ...
    return {
      ...
      stripe: !!s.payStripeEnabled && STRIPE_AVAILABLE,
    };
  }, [setting]);
  ...
  const maxCartQuantity = typeof maxPerItem === 'number' ? maxPerItem : 10;
  ...
  const [stcAccessKey, setStcAccessKey] = useState(null);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const quote = await api.shippingQuote({ city, country: addr.country, geo: addr?.geo || null });
        if (quote && quote.shipping != null) {
          setShippingCost(Number(quote.shipping) || 0);
          setShippingInfo({
            method: quote.method,
                  ...
                  <li key={i.id} className="py-2 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        type="button"
                        onClick={() => updateQuantity?.(i.id, Math.max(1, (i.quantity || 1) - 1))}
                        disabled={!updateQuantity || (i.quantity || 1) <= 1}
                      ...
                      <Button
                        ...
                        className="px-2 border"
                        type="button"
                        onClick={() => updateQuantity?.(i.id, Math.min(maxCartQuantity, (i.quantity || 1) + 1))}
                        disabled={!updateQuantity || (i.quantity || 1) >= maxCartQuantity}
      ...
      {open.realpay && !open.complete && (
        <div className="mt-10 border rounded overflow-hidden">
          ...
          <div className="p-4 space-y-6 max-w-xl">
                ...
                switch (paymentMethod) {
                  ...
                  case 'stc': {
                    ...
                    if (data?.sessionId) {
                      ...
                      setStcAccessKey(data.accessKey || null);
                      setMessage('تم إنشاء جلسة STC Pay. يرجى إتمام الدفع في تطبيق STC Pay ثم اضغط "التحقق من حالة الدفع" للتأكيد.');
                    } else throw new Error('فشل إنشاء الجلسة');
                    break;
                  }
                  case 'bank': {
                  ...
                  case 'stripe': {
                    if (!STRIPE_AVAILABLE) {
                      ...
                      setMessage('بوابة Stripe غير مهيأة في هذه البيئة.');
                      break;
                    }
                    const payload = buildOrderPayload();
                  ...
                  <Button
                    ...
                    onClick={async ()=>{
                      try {
                        setProcessing(true);
                        const oid = await ensureOrder();
                        const result = await paymentService.confirmStcPay({ orderId: oid, sessionId: stcSession, accessKey: stcAccessKey });
                        if (result?.status === 'paid') {
                          finalize();
                          setMessage('تم تأكيد الدفع عبر STC Pay بنجاح.');
                        } else if (result?.status === 'cancelled') {
                          setMessage('تم إلغاء عملية الدفع من STC Pay.');
                        } else {
                          const hint = result?.apiStatus || result?.normalized || 'جاري المعالجة';
                          setMessage(`حالة STC Pay الحالية: ${hint}. إذا اكتمل الدفع لاحقاً فسيتم تحديث الطلب تلقائياً أو يمكنك إعادة التحقق بعد لحظات.`);
                        }
                      } catch(e){
                        setMessage(e?.message || 'تعذر التحقق من حالة الدفع عبر STC Pay');
                      } finally {
                        setProcessing(false);
                      }
                    }}
                  >التحقق من حالة الدفع</Button>
                  <Button className="flex-1" variant="secondary" disabled={processing} onClick={()=>{ setStcSession(null); setStcAccessKey(null); setMessage('تم إلغاء جلسة STC'); }}>إلغاء</Button>
                </div>
              </div>
            )}
            ...
            {message && (
              <div className="text-sm text-gray-800 bg-gray-50 border border-gray-200 rounded px-3 py-2">
                {message}
              </div>
            )}
          </div>
...
const StripeCardPayment = ({ clientSecret, grandTotal, ensureOrder, finalize, setMessage, processing, setProcessing }) => {
  ...
  const handleConfirm = async () => {
    if (!stripe || !elements) {
      setMessage('خدمة الدفع غير متوفرة حالياً');
      return;
    }
    try {
      setProcessing(true);
      ...
      if (!cardElement) {
        setMessage('تعذر العثور على حقل البطاقة.');
        return;
      }
      ...
      if (error) {
        ...
        setMessage('تعذر تأكيد الدفع. حاول مرة أخرى.');
      }
    } catch (e) {
