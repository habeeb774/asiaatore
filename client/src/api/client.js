// Determine API base. We always prefer same-origin '/api' to leverage Vercel/Vite proxy
// and avoid cross-origin CORS. Only accept a relative override (e.g. '/api' or '/backend').
const DEV = import.meta?.env?.DEV;
const VITE_API_ENV = import.meta?.env?.VITE_API_URL;
let API_BASE = '/api';
if (typeof VITE_API_ENV === 'string' && VITE_API_ENV.trim()) {
  // Allow only relative paths to avoid cross-origin
  if (VITE_API_ENV.startsWith('/')) {
    API_BASE = VITE_API_ENV;
  } else if (typeof window !== 'undefined' && /^https?:\/\//i.test(VITE_API_ENV)) {
    // If absolute and same-origin, reduce to its pathname; otherwise ignore to prevent CORS
    try {
      const u = new URL(VITE_API_ENV);
      if (u.origin === window.location.origin) {
        API_BASE = u.pathname || '/api';
      }
    } catch {}
  }
}
// In development, ensure we definitely use the Vite proxy
